...
class Ruangan extends REST_Controller
{
	...
	public function index_get()
	{
		...
		if($kode_ruangan === null){
			$data_ruangan = $this->ruangan->getDataruangan();
		}else{
			$data_ruangan=$this->ruangan->getDataruangan($kode_ruangan);
		}
		if($data_ruangan){
			$this->response(
				[
					'status' => true,
					'data_person' => $data_ruangan
				],
	...
	public function index_post()
	{
		...
		if($kode_ruangan === null || $nama_ruangan === null){
			...
			if($this->ruangan->tambahruangan($data)>0){
				$this->response(
					[
						'status' => true,
	...
	public function index_delete()
	{
		...
		if($kode_ruangan === null){
			...
			if($this->ruangan->hapusruangan($kode_ruangan)>0){
				$this->response(
					[
						'status'=>true,
						'kode_ruangan'=>$kode_ruangan,
						'message'=>'data ruangan dengan kode ruangan :' . $kode_ruangan . ' berhasil dihapus'
					],
					REST_Controller::HTTP_OK
				);
			} else{
				$this->response(
					[
						'status'=> false,
						'message'=> 'data ruangan dengan kode ruangan :' .$kode_ruangan. ' tidak ditemukan'
					],
					REST_Controller::HTTP_BAD_REQUEST
				);
			}
	...
	public function index_put()
	{
		...
		if($kode_ruangan === null){
			...
			if($this->ruangan->ubahruangan($dataruangan, $kode_ruangan) > 0){
				$this->response(
					[
						'status' => true,
						'message' => 'data ruangan dengan kode ruangan :' .$kode_ruangan. ' berhasil diupdate'
					],
					REST_Controller::HTTP_CREATED
				);
			} else {
